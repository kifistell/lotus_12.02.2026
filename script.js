...
function getTrip() {
    ...
    const t = raw ? safeJSONParse(raw, null) : null;
    return (t && typeof t === "object")
        ? {
            favorites: Array.isArray(t.favorites) ? t.favorites : [],
            plan: Array.isArray(t.plan) ? t.plan : [], // [{ key, note, addedAt }]
        }
        : { favorites: [], plan: [] };
}
...
function isFavorite(key) {
    const t = getTrip();
    return t.favorites.includes(key);
}

function toggleFavorite(key) {
    const t = getTrip();
    const i = t.favorites.indexOf(key);
    if (i >= 0) t.favorites.splice(i, 1);
    else t.favorites.push(key);
    setTrip(t);
    return t;
}
...
function addToPlan(key) {
    const t = getTrip();
    if (!t.plan.some(x => x.key === key)) {
        t.plan.push({ key, note: "", addedAt: Date.now() });
        setTrip(t);
    }
    return t;
}
...
function removeFromPlan(key) {
    const t = getTrip();
    t.plan = t.plan.filter(x => x.key !== key);
    setTrip(t);
    return t;
}

function movePlanItem(key, dir) {
    const t = getTrip();
    const idx = t.plan.findIndex(x => x.key === key);
    if (idx < 0) return t;
    const to = idx + dir;
    if (to < 0 || to >= t.plan.length) return t;
    const tmp = t.plan[idx];
    t.plan[idx] = t.plan[to];
    t.plan[to] = tmp;
    setTrip(t);
    return t;
}

function updatePlanNote(key, note) {
    const t = getTrip();
    const item = t.plan.find(x => x.key === key);
    if (item) item.note = (note ?? "").slice(0, 400);
    setTrip(t);
    return t;
}

// Быстрый поиск локации по ключу (под твой places[])
function getPlaceByKey(key) {
    return (window.places || []).find(p => p.key === key || p.id === key || p.name === key) || null;
}
